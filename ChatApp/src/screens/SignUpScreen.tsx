            ...
            <KeyboardAvoidingView behavior={Platform.OS === 'android' ? "padding" : "height"} className="flex-1 justify-center items-center dark:bg-black">
                <SafeAreaView className=" p-5 justify-center items-center">
                    ...
                    <View className="w-full mt-10">
                        ...
                        value={userData.firstName} className="w-80 h-12 bg-slate-50 dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-md p-2 my-2 text-slate-500 dark:text-slate-100" placeholder="First Name" />
                        <TextInput
                            onChangeText={(text)=>{
                                setUserData((previous)=>({
                                    ...previous,
                        ...
                        value={userData.lastName} className="w-80 h-12 bg-slate-50 dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-md p-2 my-2 text-slate-500 dark:text-slate-100" placeholder="last Name" />
                    </View>
                ...
                <View className="w-full absolute bottom-36 left-56">
                    <Pressable className="w-32 mt-8 bg-green-500 dark:bg-slate-100 border border-slate-300 dark:border-slate-600 rounded-md p-2 my-2" onPress={()=>{
                        ...
                        if (validFirstName) {
                            Alert.alert("Warning", validFirstName);
                        }else if (validLastName) {
                            Alert.alert("Warning", validLastName);
                        }else{
